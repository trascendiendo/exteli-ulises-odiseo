...
const Card = () => {
  ...
  useEffect(() => {
    ...
    const prevMonth = `${getYear()}-${getMonth() - 1}`
    const month = `${getYear()}-${getMonth()}`
    let incommingPrevMonth = 0
    let incommingMonth = 0
    const fetchData = async () => {
      try {
        const res = await Apis.accounting.GetAllAccountingByMonth(month)
        if (res) {
          res.forEach(item => {
        ...
        const prevRes = await Apis.accounting.GetAllAccountingByMonth(prevMonth)
        if (prevRes) {
